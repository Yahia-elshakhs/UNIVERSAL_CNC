  ...
  Request_t requests[comm.MAX_Q_LN];
  ...
  int polled_slave[comm.MAX_Q_LN];
  ...
  void que_n(){
    ...
    void start_ser_com(int SDA_PIN,int SCL_PIN,int freq,int q_ln){
        ...
        int MAX_Q_LN =q_ln;
        int MAX_POLLED =q_ln;
        Serial.begin(freq);           // start serial for output
        Wire.begin(SDA_PIN, SCL_PIN);   // join i2c bus
      for(size_t i=0;i<comm.MAX_Q_LN;i++){
    ...
    if (Serial.available() > 0) {
      ...
      case PC2M_STATES::RECIVE_BYTES:
        ...
        if(! que_request(slave_addr,rec_bytes,slave_ln) ) {
          Serial.print("que full");//inform pc about full q
        };
        slave_addr++;
        slave_n--;
        if (slave_n==0)
  ...
  if (polled_slave_count==comm.MAX_POLLED)
  {
      //error report needed
      return;
  }
  ...
  read_i2c(sl,32);//reads data from slave and serial prints it 
